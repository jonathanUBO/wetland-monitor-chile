            ...
            <div className="p-3 z-10 bg-gradient-to-b from-black/90 to-transparent flex justify-between items-start pointer-events-none">
                <div className="flex flex-col gap-1 w-full relative">
                    <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs font-bold tracking-widest text-gray-300">
                                {mode.title} <span className="text-gray-500 font-normal">({mode.acronym})</span>
                            </span>
                        </div>
                        {/* CONTROLS REMOVED */}
                    </div>
                    ...
                    <div className="text-3xl font-mono font-medium text-white drop-shadow-md mt-1">
                        {res ? res.stats.current.toFixed(3) : '---'}
                    </div>
...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        const checkGoogle = setInterval(() => {
            ...
            if (window.google && window.google.accounts) {
                ...
                console.log('[GEE] Scripts cargados.');
            }
        }, 500);
    ...
    const handleAnalyze = async () => {
        ...
        let aoiGeometry;
        if (selectedWetland) {
            // Use exact polygon from inventory if available, otherwise fallback to bbox
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white flex overflow-hidden font-sans">
            ...
            <aside className="w-80 h-screen p-6 flex flex-col gap-6 z-20 pointer-events-auto bg-black border-r border-white/5 overflow-y-auto custom-scrollbar">
                ...
                <GlassPanel id="control-panel" className="p-4 flex flex-col gap-3 mt-auto">
                    ...
                    <button
                        ...
                        title={!selectedWetland ? 'Selecciona un humedal primero' : !accessToken ? 'Conéctate a GEE primero' : ''}
                    >
            ...
            {/* MAIN DASHBOARD */}
            <main className="flex-1 h-screen overflow-hidden flex flex-col relative bg-gradient-to-br from-gray-900 to-black p-4 gap-4">
